...
void designer_PVE::update_saveOK()
{
	saveOK = false;
}

bool designer_PVE::isOver()
{
	return isover;
}

void designer_PVE::onKey(key_msg key)
{
...
void designer_PVE::key_direction(key_msg key)
{
...
void designer_PVE::key_WASD(key_msg key)
{
...
void designer_PVE::key_Space(key_msg key)
{
	if (key.key == key_space&&key.msg == key_msg_down)
	{
		...
		else if (cursor2_x == Win_W - 136 + 4 && cursor2_y == map_y + 12 + 80)
		{
			if (isOK_4_home())
			{
				tempmap.setmapmember_4_plus(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20, home);
				issave = false;
			}
		}
		else if (cursor2_x == Win_W - 136 + 8 + 40 && cursor2_y == map_y + 12 + 80)
		{
			if (isOK_4() && enemynum < 3)
			{
				tempmap.setmapmember_4_enemy(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20);
				enemynum++;
				issave = false;
			}
		}
		...
		if (cursor2_x == Win_W - 136 + 4 && cursor2_y == map_y + 20 + 160)
		{
			if (bigsize)
			{
				...
				if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) == enemyborn_h)
				{
					tempmap.setmapmember_4(0, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20, land);
					tempmap.setmapmember_4(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20, land);
					enemynum--;
...
void designer_PVE::key_ESC(key_msg key)
{
...
void designer_PVE::key_Shift(key_msg key)
{
...
void designer_PVE::rend_information()
{
	//文字
	setcolor(WHITE);
	setfont(25, 0, "楷体");
...
void designer_PVE::rend_bar(int x, int y, int w, int h, int n, color_t color)
{
	setfillcolor(color);
	bar(x - n, y - n, x + w + n, y);
...
bool designer_PVE::isOK_4_player1()
{
	if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != home)
		if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != home_h)
			if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != enemyborn)
...
bool designer_PVE::isOK_4_player2()
{
	if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != home)
		if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != home_h)
			if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != enemyborn)
...
bool designer_PVE::isOK_4_delete()
{
	if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != home)
		if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != home_h)
			if (tempmap.getmember(1, (cursor1_x - map_x) / 20, (cursor1_y - map_y) / 20) != player1born)
